...
Record the result with `python3 ./.trellis/scripts/task.py set-prd-status <task-dir> confirmed` or `... override`. While `meta.prd_status` is `draft` (or missing), do NOT start implementation and do NOT run `task.py start`.
